...
export function getHour(time){
    return parseInt(time / (60000 * 60)) // 60 초(1분) * 60 = 60분 = 1시간
}

// 단위 변경 함수 (ms to m)
// 분단위 변환 후 60*N분이 넘었을 경우 빼줌
export function getMinute(time){
    return parseInt(time / 60000) - 60 * getHour(time) // 76분 - 60분 = 16분
}

// 단위 변경 함수 (ms to s)
// 초단위 변환 후 60*N초 넘었을 경우 빼줌
export function getSecond(time){
    return parseInt(time / 1000) - 60 * getMinute(time)
}
...
export function splitProblemToNewline(text){
    ...
    newLine = newLine.replace("(나)", "<br/>(나)")
