...
// import 유틸
import { getNow, getScoring, sendProblemRecommend } from './utils';
	...
	try {
		...
		if (problem.PRB_SECT == "LS" || problem.PRB_SECT == "RD") {
			...
			if(userProblem.SCORE == undefined){
				...
				Alert.alert(problem.PRB_NUM, "채점이 완료되었습니다.")
...
export async function setLeveltestColl(problems, user_id, user_level, userIndex){
	...
	for(let i=0; i<problems.length; i++){
		...
		await historyColl.add(DATA)
	}
	
	// 레벨테스트 문제를 모두 풀었을 경우 추천문제 생산
	if(userIndex == 10){
		await sendProblemRecommend(user_id)
