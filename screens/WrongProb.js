...
const problemStructure = (loadedProblem) => {
    let question = []
    ...
    return (
      <View>
        <ScrollView  key = {props.key}>
            {
                problemStructure(props.problem)
            }
       ...
       <TouchableOpacity onPress = {() => {setClick(4)}} disabled = {subBtn} style = {[styles.button, {backgroundColor: props.problem.choice ? setBtnColorStop(4) :setBtnColor(4)}]}>
        ...
        <View style = {{flexDirection: "row"}}>
            <TouchableOpacity onPress={()=>{props.setNextBtn(props.nextBtn-1)}} disabled = {props.nextBtn == 0} style = {[styles.button, {backgroundColor: props.nextBtn == 0 ? "#D9D9D9" : "#94AF9F"}]}>
                <Text>
                    PREV
                </Text>
            </TouchableOpacity>
            ...
            {!subBtn ? 
            (<TouchableOpacity onPress = {() => {setSubBtn(true)}} disabled = {click==0} style = {[styles.button, {backgroundColor: click == 0 ? "#D9D9D9" : "#94AF9F"}]}>
                <Text>
                    SUBMIT
                </Text>
            </TouchableOpacity>): 
